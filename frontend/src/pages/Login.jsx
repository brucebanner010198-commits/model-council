...
import { Users, Sparkles, Mic, Gavel, Mail, Loader2, CheckCircle2 } from "lucide-react";
...
export default function Login() {
  ...
  const sendMagic = async (e) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.detail || "Could not send the link");
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <div className="grain relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
        ...
        {sent ? (
          <div data-testid="magic-sent" className="flex items-center gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/5 px-4 py-3.5 text-sm text-emerald-300">
            <CheckCircle2 className="h-4 w-4" /> Link sent to {email}. Check your inbox.
            <button onClick={() => setSent(false)} className="ml-auto text-xs text-zinc-500 hover:text-white">use another</button>
          </div>
        ) : (
          <form onSubmit={sendMagic} className="flex gap-2">
            <div className="relative flex-1">
              <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-500" />
              <input data-testid="magic-email-input" type="email" value={email} onChange={(e) => setEmail(e.target.value)}
            ...
            <button data-testid="magic-link-btn" type="submit" disabled={sending}
              className="flex items-center justify-center gap-2 rounded-full border border-white/15 px-5 h-12 text-sm text-zinc-200 hover:bg-white/5 disabled:opacity-50 transition-colors duration-200 whitespace-nowrap">
              {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Mail className="h-4 w-4" />} Email me a link
            </button>
          </form>
        )}
